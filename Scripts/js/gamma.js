	...
	var $window = $( window ),
		...
		_layout = function( $items ) {
			...
			$items.each( function() {
				...
				// get photo
				var photo;
				for (var p in Gamma.photos) {
					if (Gamma.photos[p].id == $item[0].innerText) {
						photo = Gamma.photos[p];
					} 
				}
				var template = function (photo) {
					return 	'<div class="container" style="display:block;">' +
							'<div class="panel panel-default" width="120"><div class="panel-heading">' +
					 		photo.nombre +
		...
		_singleview = function() {
			$window.trigger('show-gamma-info');
			var id = $( this ).index();
			_saveState( id );
